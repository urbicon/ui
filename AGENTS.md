...
- `docs:fences:lint` — compiles the opted-in `ts` fences of the package READMEs and the six tarball docs with raw `tsc` against a throwaway consumer project linked to the built packages, so imports resolve through the published exports map — the class of error `examples:lint` cannot see. Opt-in per `<!-- typecheck -->` on the line above the fence; `$env`/`$lib` ambient derived from the fences' own imports. Needs `build:packages` first; runs in `gates`.
