...
- `typesref:lint` — a component page documents its types in two hand-written halves and each fails silently alone: without `types=` on `<ApiReference>` the `knownTypeNames` set is empty and `tokenizeTypeExpression` renders every type name in the Type column as unlinked text; without a `<TypesReference>` the `#type-<Name>` links that prop generates point at a section that does not exist (four pages shipped that way). Enforces both directions over every route dir with a generated `api.ts`, against the types that pass `t.exported && (!t.owner || t.owner === name)`. **Resolves** the attribute rather than counting it — `types={[]}`, `types={undefined}` or a hand-written demo array are reported as dead, and an expression it cannot resolve is reported too; demo instances inside `CodeExample`/`PlaygroundConfigurator` are blanked first. `PENDING_STAGE3` holds the pre-existing backlog (93 pages, #114 stage 3 empties it) and `NO_PAGE` the page-less api dirs; both stale directions are errors and `PENDING_STAGE3` may only shrink. Reads the generated api.ts — `docs:gen:all` first. **Not** `types:guard` (that one is declaration emit)
