...
- **A fresh worktree needs a build before anything else.** `bun install`, then `bunx --bun svelte-kit sync` (blocks, docs-app), then `bun run build:packages` (staged since 2026-08-12: shared-types → i18n → blocks first, then the parallel sweep — the flat `--filter='./packages/*'` run built `docs` before `blocks/dist` existed, and tsc then silently emits every `*Variants` export as `any`: svelte-package still exits 0, and the docs-app check fails with ~355 phantom errors). Without it: the docs-app dev server won't start, `docs-app run check` reports hundreds of missing `@urbicon-ui/…` and `./api` modules (`api.ts` is git-ignored — run `docs:gen` too), and `blocks run check` shows ~5 phantom `Snippet` errors plus a failing `run test`, all because `@urbicon-ui/i18n` has no `dist/` yet.
