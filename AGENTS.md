...
- `variants:lint` — dead-token guard over all tv() configs, plus the bucket-agreement pass: it asks the Tailwind compiler which properties each class declares and fails on a family `BUCKET_PATTERNS` has no pattern for (the resolver goes silent there and the stylesheet's emit order decides) or a bucket whose classes write different properties. Two populations, because their blind spots are disjoint — the missing-family half reads the shipped classes, the differing-properties half reads Tailwind's whole class list as well (`getClassList()` names no arbitrary value and no variant-prefixed form, so `stroke-[2px]` is only in the first) · `imports:lint` — cross-component import guard (see Key Architecture Decisions)
- `consumer-css:check` — a package can ship `.svelte` whose Tailwind classes no consumer ever compiles, and `apps/docs` cannot notice: it scans every `src/lib` whole and is not a consumer (#314 found 19 such classes). Packs every published package that ships `.svelte` (`bun pm pack`), installs the tarballs into a throwaway `node_modules`, and asks Tailwind's own `compile()` → `Scanner` → `candidatesToCss()` whether the shipped stylesheet reaches every class in the shipped markup — no class regex of its own. A package that ships styled markup but exports no `./style/index.css` is reported as exactly that. Runs in the `build` job (needs `dist/`), ~1 s; its positive controls (`bun test scripts/consumer-css-check.test.ts`, in the `test` job) reproduce the pre-#314 `@source` lines on the unpacked copies and expect that finding back. All five packages report 0 today; the first run had auth at 10, before it exported `./style/index.css`
