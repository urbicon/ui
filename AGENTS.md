...
- `variants:lint` — dead-token guard over all tv() configs, plus the bucket-agreement pass: it asks the Tailwind compiler which properties each shipped class declares and fails on a family `BUCKET_PATTERNS` has no pattern for (the resolver goes silent there and the stylesheet's emit order decides) or a bucket whose classes write different properties · `imports:lint` — cross-component import guard (see Key Architecture Decisions)
