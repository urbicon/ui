...
- `registry:lint` — a docs page is hand-registered in three places (sidebar `navigation.ts`, `componentLinks`, the recipes cookbook) and forgetting one is silent: the page never appears in the sidebar, `@related` chips to it are dropped without a word (`buildRelatedLinks` skips unknown names), recipe chips fall back to `#`. Checks that every catalogue component with a `+page.svelte` is in `componentLinks` and points at its own page, that every catalogue component **without** one is a decision rather than an oversight, that every href and every recipe/showcase chip resolves, that every page is in the sidebar and every recipe in the cookbook. A deliberately unlisted page needs an `UNLISTED` entry, a deliberately page-less component a `PAGELESS` entry, both with a reason, in `apps/docs/scripts/registry-lint.ts`; stale entries are errors too (same contract as `imports:lint`). Reads the generated catalogs — `docs:gen:all` first
- `examples:budget` — the 2–4 `<CodeExample>` budget per component page (`docs/DocsPageGuide.md` XC-6), which existed only as prose until 2026-08; measured against `main` it reported 17 pages over and 2 under. Runs in the `gates` job. **Not** `examples:lint` (that one type-checks `@example` JSDoc). Counts `examples` + the Mint section together — a standalone Mint section must not buy extra budget — matching the id as `/^mints?$/i` because pages spell it both ways (`checkbox` singular, `segment-group` plural), and never counts `customization` / `installation` / `api` / `types`. A component-specific section may not itself exceed the budget (that is how `badge`'s five-demo `patterns` section hid); a justified one takes an `OVERSIZE_OK` entry. A page with no examples section needs a `NO_EXAMPLES` entry — which excuses the page total, not the per-section ceiling. Stale entries are errors in both maps, and a `MIN_PAGES` floor stops a drifting parser reporting clean over an empty set
...
- `typesref:lint` — a component page documents its types in two hand-written halves and each fails silently alone: without `types=` on `<ApiReference>` the `knownTypeNames` set is empty and `tokenizeTypeExpression` renders every type name in the Type column as unlinked text; without a `<TypesReference>` the `#type-<Name>` links that prop generates point at a section that does not exist (four pages shipped that way; PR #140 wired them). Enforces both directions over every route dir with a generated `api.ts`, against the types that pass `t.exported && (!t.owner || t.owner === name)`. **Resolves** the attribute against a canonical shape rather than counting it, and remembers **which** generated module it reads: `types={[]}`, `componentData?.props ?? []` (the copy-paste slip from the line above), `.types.slice(0,0)` and a bare `demo.x` are all reported, an unresolvable expression is reported too, and the two halves must read the **same** `componentData`, bound from the page's **own** `'./api'` — feeding `<ApiReference>` from `panelData.types` while `<TypesReference>` shows `componentData.types` is an error naming both, and pointing both at a neighbour (`'../tooltip/api'`, the wrong-path slip) is an error too, because the page would document someone else's types while its own land nowhere. Bindings are read from `+page.svelte` alone, never from the spliced markup — splicing merges import scopes, and a sibling's legitimate `'./api'` otherwise launders a foreign binding of the same name. Demo instances inside `CodeExample`/`PlaygroundConfigurator` are blanked first. A page whose types all vanish from the filter is an error, not an exemption. `STAGE_3_ROSTER` is the **frozen** backlog (93 routes, permanent keys, value flips `PENDING`→`WIRED`; `ROSTER_SIZE` is checked for equality — a ceiling would let stage 3 free a slot for every page it fixes) and `NO_PAGE` the page-less api dirs (see #141 — the Guide surfaces' types land on no page, and the reason string says so). Reads the generated api.ts — `docs:gen:all` first. **Not** `types:guard` (that one is declaration emit)
...
- [docs/COMPONENT-FAMILIES.md](docs/COMPONENT-FAMILIES.md) – Seven-family taxonomy (Action/Form/Navigation/Container/Feedback/Identity/Conversation) — ARIA, tier behaviour, border-token source per family
...
- [docs/VARIANT-CONTRACT.md](docs/VARIANT-CONTRACT.md) – What each `variant` value means across the library: the surface ladder, Card/Alert/Accordion/Table variants, border tokens, naming history. Referenced from the `*.variants.ts` files it governs. **Symlink into `packages/blocks/docs/`, ships in the blocks tarball** — keep it public-appropriate.
...
- **Every issue carries four label axes**, set when it is opened — a new issue with only a `debt:` label erodes the taxonomy the 2026-08-03 pass established:
  ...
  | `prio:P1\|P2\|P3` | P1 = hits shipped code at a consumer, or blocks other work | the order |
