...
- Root is a Bun workspace (`"private": true`). Source in `packages/*`:
  ...
  - `shared-types`: docs-tooling types (playground, docs-config, navigation) — a peer of `@urbicon-ui/docs` and a dependency of `docs-gen`, nothing else; the runtime packages reference its `globals` augmentation only while type-checking their own sources
