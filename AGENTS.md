...
- `examples:budget` — the 2–4 `<CodeExample>` budget per component page (`docs/DocsPageGuide.md` XC-6), which existed only as prose until 2026-08 and had drifted to 15 pages over and 4 under. **Not** `examples:lint` (that one type-checks `@example` JSDoc). Counts `examples` + `mint` together — a standalone Mint section must not buy extra budget — and never counts `customization` / `installation` / `api` / `types`. A page with no examples section needs a `NO_EXAMPLES` entry with a reason; stale entries are errors, and a `MIN_PAGES` floor stops a drifting parser reporting clean over an empty set
