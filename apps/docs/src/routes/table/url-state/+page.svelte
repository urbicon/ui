  ...
  <div class="space-y-12">
      ...
      <div class="border-border-subtle bg-surface-elevated mt-8 rounded-2xl border p-6">
        ...
        <ul class="text-text-secondary list-inside list-disc space-y-2 text-sm">
          <li>
            ...
            <em>every</em>
            axis — including the ones it filled in from the defaults — so a table wired to it silently
            ignores <code class="text-text-primary">persistenceConfig</code> and all
            <code class="text-text-primary">initial*</code> props on any URL. It is a snapshot to seed
            a fetch with, which is what the server example below uses it for.
          </li>
          <li>
            ...
            <code class="text-text-primary">sortColumn</code>/<code class="text-text-primary"
              >sortDirection</code
            >
            for a baked-in <code class="text-text-primary">initialSort</code>) so the baseline
            matches the state the table starts in — otherwise its opening view writes params
            describing itself.
          </li>
        </ul>
      </div>
    ...
    <Section id="precedence" title="URL, Storage, Seed">
      ...
      <div class="border-border-subtle bg-surface-elevated rounded-2xl border p-6">
        <ol class="text-text-secondary list-inside list-decimal space-y-3 text-sm">
          <li>
            ...
            controls its axis. This is a derivation, not a write, which is why it also resolves during
            server rendering.
          </li>
          <li>
            <strong class="text-text-primary"
          ...
          <li>
            ...
            <code class="text-text-primary">initialSelectedIds</code>. They fill only what neither
            of the two above supplied.
          </li>
        </ol>
      </div>
      ...
      <div class="border-border-subtle bg-surface-elevated mt-6 rounded-2xl border p-6">
        ...
        <ul class="text-text-secondary list-inside list-disc space-y-2 text-sm">
          <li>
            <strong class="text-text-primary">Empty is a value, at every layer.</strong> A URL that
            carries <code class="text-text-primary">?sort=</code> says "no sort" — the seed does not slip
            past it. The same rule already governed storage: an axis the reader cleared restores cleared.
          </li>
          <li>
            <strong class="text-text-primary"
              >Column visibility and column order are not axes.</strong
            >
            They are presentation, not selection, so they stay in
            <code class="text-text-primary">localStorage</code> and never enter the URL — nobody wants
            to share a link that hides columns on the other end. The server renders every column.
          </li>
          <li>
          ...
          <li>
            ...
            <code class="text-text-primary">viewState</code> re-reads the URL rather than capturing
            it once, so navigating back to an address without
    ...
    <Section id="between-visits" title="Keeping It Between Visits">
      <p class="text-text-secondary mb-6 text-sm">
        By default a controlled axis is neither read from storage nor written to it: while the URL
        carries the state, the URL <em>is</em> the state. What that does not survive is opening the page
        from a bare link — nothing was stored, so the reader starts clean. For a business table that is
        usually the wrong answer, since "my filters are still there tomorrow" is expected.
      </p>
    ...
    <Section id="server" title="What the Server Renders">
      <p class="text-text-secondary mb-6 text-sm">
        ...
        <code class="text-text-primary">$effect</code> never runs during server rendering, so any
        view state ingested in one is absent from the prerendered HTML — the server would send an
        unsorted, unfiltered table that the client swaps out on hydration, with the reader watching.
        A <code class="text-text-primary">$derived</code> is evaluated on the server, so the linked view
        is in the markup that arrives.
      </p>
      <p class="text-text-secondary mb-6 text-sm">
