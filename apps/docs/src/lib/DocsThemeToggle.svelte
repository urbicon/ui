<script lang="ts">
  ...
  // Storage key + values mirror the head script in app.html: 'editorial' is the
  ...
  type Variant = 'editorial' | 'library';

  // SSR renders the shipped default (editorial). The app.html head script has
  // already applied the correct `.docs-editorial` class on <html> from
  // localStorage before first paint, so onMount only syncs this control's active
  // segment — no DOM write here, and no hydration mismatch (the segment flips
  // client-side after mount, exactly like ThemeSwitcher's icon).
  ...
  // target here) and persists the choice, dropping the key on editorial so the
  // default stays implicit.
  function persist(next: string) {
    document.documentElement.classList.toggle('docs-editorial', next === 'editorial');
    if (next === 'library') {
...
<div>
  ...
  >
    <SegmentItem value="editorial">{ta('chrome.themeVariant.editorial')}</SegmentItem>
