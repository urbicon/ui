...
// und das Hero-Fixture (`/test-fixtures/landing-hero`).
//
...
export function loadLandingRows(): { rows: HeroRow[]; foundationGz: number } {
  ...
  const rows: HeroRow[] = catalog.map((entry) => ({
    ...
    // Der Katalog liefert die Zahl für jede Zeile; die Null greift nur, wenn
    // ein Eintrag sie verliert — dann steht sie da und fällt auf, statt still
    // eine plausible Zahl zu erfinden.
