...
- **A slot name in filter position** — `Exclude<XSlots, 'base'>` where a compound part does not own one slot of a shared config, `Extract<XSlots, …>` where it owns only some. Six components narrow that way today; the set is every `Exclude<`/`Extract<` over a `*Slots` type in a `slotClasses` prop.
