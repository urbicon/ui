...
A **wrapper** is the one component that names axes it did not write: `wrapperActiveProps` gives it every axis of the inner config, at `undefined` where the caller wrote nothing, because a wrapper stands in for the whole inner component. An item beside its siblings does not, and must not.
