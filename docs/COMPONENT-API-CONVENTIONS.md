...
**Writing your own wrapper** takes the same two steps, and the exported `wrapperActiveProps` is the second one: destructure `preset` and `slotClasses` out of the rest spread, then hand the inner component `slotClasses={resolveSlotClasses(config, 'YourWrapper', preset, wrapperActiveProps(innerVariants.config, writtenProps), slotClasses, innerVariants.config)}` and no `preset`. A wrapper sees only what its caller wrote, so `wrapperActiveProps` narrows that to the keys the inner component's config declares as axes — a rule may not key on `label`, which the inner condition object never carries either. The axes the caller left out are filled in by `resolveSlotClasses` itself, from the same `innerVariants.config`, which is why the wrapper and the component it wraps answer one rule the same way. Its JSDoc names the three axis kinds it still gets wrong (issue #360).

When `unstyled` is `false`, `slotClasses` values are merged with the default tv() classes; when `unstyled` is `true`, they replace them entirely. Components resolve the value through `resolveSlotClasses(blocksConfig, 'Name', preset, variantProps, slotClassesProp, nameVariants.config)`, which composes the full cascade (weakest → strongest):
...
Two rules follow from the two halves reading one config:

- **A boolean axis carries its raw value**, never `x || undefined`. The config's `defaultVariants` supply the axis when the object leaves it out, so both spellings render identically — but only the raw value makes `{ disabled: false }` a rule anyone can write. The three that a project-wide rule reaches for are `disabled`, `readonly` and `error`; `provider/boolean-conditions.svelte.test.ts` asserts both sides of each, for every component declaring the prop.
