...
- **The accent belongs to the primary action.** In the library's defaults the accent (`primary`) sits only on the one thing a surface asks you to do — the confirm button of a dialog, the palette's "Enter runs this" cursor, the active tab. It does not sit on an empty state's icon disc, a required-field marker or a neutral confirmation: those defaults are neutral, and a product that wants the accent there opts in — `confirmIntent="primary"` on the dialog, `slotClasses` or `overrides` on the rest. Products whose accent carries a meaning ("light and rating", "the one word you press") otherwise pay for every accent-by-default before they write a feature.
