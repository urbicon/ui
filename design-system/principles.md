...
- **The accent belongs to the primary action.** In the library's defaults the accent (`primary`) sits only on the one thing a surface asks you to do: the palette's "Enter runs this" cursor, the active tab. A dialog's confirm button follows the dialog's own `intent` — accent only where you asked for `intent="primary"`, neutral where you asked for `neutral`. It does not sit on an empty state's icon, a required-field marker or a neutral confirmation: those defaults are neutral, and a product that wants the accent there opts in — `confirmIntent="primary"` on the dialog, `slotClasses` or `overrides` on the rest. Products whose accent carries a meaning ("light and rating", "the one word you press") otherwise pay for every accent-by-default before they write a feature.
