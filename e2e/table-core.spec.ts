...
test.describe('Table core flows', () => {
  ...
  // A column header belongs over the same edge as the cells beneath it — so the
  // assertion compares the two, never a header against its own `<th>`. Measured
  // against the `<th>` alone, three different half-fixes all look right: a
  // `justify-*` that never leaves `titleContent` (which moves nothing at all),
  // one on `titleContainer` while the header menu is still its sibling (which
  // stops ~40px short), and one that only ever got applied to `right`.
  //
  // All three alignments run, because none of those half-fixes was specific to
  // `right` — the menu held 40px of every header cell.
  test('a column header sits over the same edge as its cells, at every alignment', async ({
    page
  }) => {
    await setupPage(page);
    ...
    // Read off the rendered TEXT, via a Range over the deepest element holding
    // it. Measuring the boxes compares things that are not the same thing: the
    // cell's inner box carries 8px of padding the header's does not, so two
    // perfectly aligned columns read as 8px apart. No fallback to an enclosing
    // box — a probe that quietly measures the wrong element is how two earlier
    // versions of this test passed over a 40px misalignment.
    const textEdges = (locator: ReturnType<typeof standard.locator>, label: string) =>
      locator.evaluate((el, name) => {
        ...
        return { left: rect.left, right: rect.right, centre: (rect.left + rect.right) / 2 };
      }, label);

    const columns = [
      { id: 'name', nth: 1, align: 'left' as const },
      { id: 'category', nth: 2, align: 'center' as const },
      { id: 'score', nth: 3, align: 'right' as const }
    ];

    for (const column of columns) {
      const header = await textEdges(
        standard.getByTestId(`column-header-${column.id}`),
        `the ${column.id} header`
      );
      const cell = await textEdges(
        standard.locator(`tr[data-row-index] td:nth-child(${column.nth})`).first(),
        `the ${column.id} cell`
      );

      // Compare the edge the alignment is *about*: a centred column's text
      // widths differ, so its left and right edges legitimately do not match —
      // only its centre line does.
      const edge = column.align === 'center' ? 'centre' : column.align;
      expect(
        Math.abs(header[edge] - cell[edge]),
        `${column.id} (${column.align}) header vs cell on the ${edge} edge`
      ).toBeLessThan(2);
    }

    // …and it survives the chrome a header grows when it is in use. Sorting
    // adds a chevron beside the title; on a right-aligned column that chevron
    // has to take the leading side, or it pushes the title off the edge its
    // numbers align to.
    await standard.getByTestId('column-header-score').locator('[role="button"]').click();
    await expect(standard.getByTestId('column-header-score')).toHaveAttribute(
      'aria-sort',
      'ascending'
    );

    const sortedHeader = await textEdges(
      standard.getByTestId('column-header-score'),
      'the sorted score header'
    );
    const sortedCell = await textEdges(
      standard.locator('tr[data-row-index] td:nth-child(3)').first(),
      'the sorted score cell'
    );
    expect(
      Math.abs(sortedHeader.right - sortedCell.right),
      'score header vs cell once a sort chevron is present'
    ).toBeLessThan(2);
  });
