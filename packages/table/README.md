...
- **A column's first sort step is ascending, unless the column says otherwise.** `sortDescFirst: true` on a column definition — a date, a rating, a count — starts it descending, and the header cycle runs first direction → opposite → unsorted. Every surface that picks a direction _for a column the reader just chose_ reads it: the header click, and the filter bar's sort panel, which is the only sort control the mobile card layout has. Controls that **name** a direction do not — the header menu's two items, that panel's ascending/descending segments, the wide bar's `column · direction` options. It is never inferred from `dataType` (TanStack starts numeric columns descending on its own).
