...
export const TableColumns = {
   ...
   * (152px) is the one value that holds at every size, with 2px to spare.
   *
   ...
   * column with `extraActions` needs its own `width`: 38px per extra button
   * (34 + the 4px gap).
   *
   ...
   * `e2e/table-actions-budget.spec.ts`, which measures what the engine actually
   * laid out and holds the used column width against this declaration.
   */
  actions: <Item>(
    title = 'Actions',
