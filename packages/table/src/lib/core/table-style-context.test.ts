...
// The real `table` slot: base `w-full border-collapse`, and from the default
// `cardsBelow` step it also carries `min-w-[42rem]` — the exact base utility a
...
describe('resolveSlotClass', () => {
  it('folds a slotClass over a conflicting base/variant utility (the fix)', () => {
    // Before the fold refactor this returned both `min-w-[42rem]` and `min-w-0`
    ...
    expect(result).not.toContain('min-w-[42rem]');
  ...
  it('keeps base + variant classes when no slotClass is given', () => {
    ...
    expect(result).toContain('border-collapse');
    expect(result).toContain('min-w-[42rem]');
  });
  ...
  it('folds both slotClass and extra over their conflicting base/variant buckets', () => {
    ...
    // `text-sm`; extra `min-w-0` beats the step's `min-w-[42rem]`.
