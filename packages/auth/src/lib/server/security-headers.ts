...
export interface SecurityHeadersConfig {
   ...
   * derived from `jwt.cookieSecure !== false`); over plain HTTP browsers
   * ignore the header anyway and emitting it during local dev would pin
   * `localhost` to HTTPS. Behind a TLS-terminating proxy keep
   * `jwt.cookieSecure` truthy (the standard setup) so HSTS stays enabled.
   *
...
export interface ApplySecurityHeadersOptions extends SecurityHeadersConfig {
   ...
   * `createAuthHandle` derives this from `config.jwt.cookieSecure !== false`.
   */
  secure?: boolean;
}
