 ...
 */
function sseStream(status = 200, honourAbort = true): FakeStream {
...
describe('NotificationListener (component)', () => {
  ...
  it('resets the attempt counter once a stream opened, and reconnects when the server closes it', async () => {
    ...
    // Counter was reset by the successful open: this is attempt 1 again (1 s), not 2 (2 s).
    expect(onReconnect).toHaveBeenLastCalledWith(1);
    await advance(1000);
    expect(fetchMock).toHaveBeenCalledTimes(3);
  });
