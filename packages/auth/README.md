...
See [`prisma/auth-schema.prisma`](./prisma/auth-schema.prisma) for the reference schema. Models: `User`, `Invitation`, `PushSubscription`, `Notification`, `NotificationType`, `NotificationPreference`, `Passkey`, `TwoFactorBackupCode`. Copy/merge into your app's schema.
...
- **Federated Identity / SSO** — `createFederatedAuthHandle()`: ES256 + JWKS, building on the existing ECDSA/RSA primitives.
