...
of it with `null` too. The three limiters that guard a secret — `login`,
`twoFactor`, `passkeyAuth` — count attempts, and a success refunds the slot its
request took: behind a shared address they brake failures only, and twenty
colleagues signing in from one office spend nothing. A refund, not a reset — a
success gives back exactly what it cost, so an attacker with an account of his
own cannot clear the address's budget between guesses at other accounts.
