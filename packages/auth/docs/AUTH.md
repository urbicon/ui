...
**One factory per feature.** A feature that serves several routes ships exactly one bundled factory, `create*Handlers(deps, …)`, taking `AuthDeps` and returning one entry per route group: `createSessionsHandlers(deps)`, `createTwoFactorHandlers(deps)`, `createPasskeyHandlers(deps, webauthn)`, `createInvitationHandlers(deps, options)`. A single-route feature is one `create*Handler`. The notification stack is the one second style, and a deliberate one: its factories are built around the service or repository they serve — `createNotificationsHandlers(service)`, `createPreferencesHandler(repo, registry)`, `createPushSubscriptionHandler(repo)`, `createStreamHandler(sse)` — not around `AuthDeps`.
