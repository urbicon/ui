...
**One factory per feature.** A feature that serves several routes ships exactly one bundled factory, `create*Handlers(deps, …)`, taking `AuthDeps` and returning one entry per route group: `createSessionsHandlers(deps)`, `createTwoFactorHandlers(deps)`, `createPasskeyHandlers(deps, webauthn)`. A single-route feature is one `create*Handler(deps)`. The exceptions: `createInvitationHandlers(deps, options)` returns flat `POST` / `GET` / `DELETE` methods that mount across two routes (`/api/invitations` and `/api/invitations/[id]`); `createJWKSHandler(config)` and `createPushKeyHandler(vapidPublicKey)` take only the configuration they serve; and the notification stack is a deliberate second style, built around the service or repository each factory serves — `createNotificationsHandlers(service)`, `createPreferencesHandler(repo, registry)`, `createPushSubscriptionHandler(repo)`, `createStreamHandler(sse)` — not around `AuthDeps`.
...
What the package deliberately does not do. Each was decided as scope, not left as a gap to
close; reopening one takes a deliberate scope decision rather than a feature request.

- **OAuth / social login.** No provider plumbing and no provider adapters: provider APIs
  drift, which makes every provider a standing maintenance cost, and a provider SDK is the
  opposite of zero-dependency. Put your own buttons in the `header` snippet of
  `<LoginPage>` / `<RegisterPage>` and drive the flow with an OAuth 2.0 / OIDC client of your
  choice, such as Arctic.
- **Organisations, teams, RBAC, SAML, SCIM.** The package carries one `role` string per user
  and nothing above it. Tenancy, permission models and enterprise provisioning are built by
  the consumer, over its own repositories.
- **Acting as an OAuth / OIDC provider for third parties.** The package signs in your users to
  your apps; it issues no tokens to other people's clients.

**First-party SSO is not a non-goal.** Sign-in shared across your own apps under one parent
domain ships as [Federated Identity (SSO)](#federated-identity-sso). Opening your own apps to
AI clients — an OAuth 2.1 authorization server plus an MCP dispatcher — is planned as a
package of its own, outside this one
([#415](https://github.com/urbicon/ui/issues/415)), so the third non-goal above stays true.
...
never in a sign-in wave. One key is identified differently from all the others:
`passkeyRegister` counts per **authenticated user id**, not per address —
enrolment sits behind a session, so a per-user key cannot be dodged by rotating
addresses, and it does not brake an office enrolling from one NAT address.
...
| `passkeyRegister`                                                       | **1 min** | 10  | the authenticated half of the same feature, keyed by **user id**. Enforced on `registrationVerify`, the call that creates the credential row. It caps the rate at which one account adds credentials — and with them the `excludeCredentials` / `allowCredentials` payloads later ceremonies carry. **No per-user credential cap stands beside it**, so this rate is the only bound on either: ten a minute is 600 rows an hour for an account that keeps at it. A brake, not a ceiling |
...
attempt **per user**, because it has no limiter ([the rule for authenticated
writes](#rate-limiting-lockout--route-scope-deliberate-trade-offs)) and one
authenticated caller could otherwise fill the sink. So the `enable` lines count
the users the rotation reached, not the attempts they made. Watch that sink: the
response is a `500 totp_secret_unreadable` built with `authError`, which
**returns** a Response rather than throwing `error()`, so SvelteKit's
`handleError` never fires and a Sentry integration hooked there sees nothing.
...
- **Same trust domain:** cookie-based federation needs a shared parent domain (`cookieDomain`). Cross-domain SSO (redirect-based token handoff, OIDC) is out of scope — as is acting as an OAuth/OIDC provider for third parties (see [Non-goals](#non-goals)).
...
| Authenticated write                                                                                                                                                                                                       | Limit                                    | Key     | Clause                                                                                                                                                                                                                   |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `createChangePasswordHandler` `POST`                                                                                                                                                                                      | `rateLimit.changePassword`, 5 / 15 min   | IP      | secret verification                                                                                                                                                                                                      |
| `createChangeEmailHandler` `POST`                                                                                                                                                                                         | `rateLimit.changeEmail`, 5 / 15 min      | IP      | secret verification, third-party cost                                                                                                                                                                                    |
| `createDeleteAccountHandler` `POST`                                                                                                                                                                                       | `rateLimit.deleteAccount`, 5 / 15 min    | IP      | secret verification                                                                                                                                                                                                      |
| `createTwoFactorHandlers` `disable`                                                                                                                                                                                       | `rateLimit.twoFactorDisable`, 5 / 15 min | IP      | secret verification                                                                                                                                                                                                      |
| `createRefreshHandler` `POST`                                                                                                                                                                                             | `rateLimit.refresh`, 30 / min            | IP      | secret verification                                                                                                                                                                                                      |
| `createPasskeyHandlers` `registrationVerify`                                                                                                                                                                              | `rateLimit.passkeyRegister`, 10 / min    | user id | state growth (the call that creates the credential row)                                                                                                                                                                  |
| `createPreferencesHandler` `PUT`                                                                                                                                                                                          | its own `rateLimit` option, 30 / min     | user id | state growth                                                                                                                                                                                                             |
| `createPushSubscriptionHandler` `POST`                                                                                                                                                                                    | its own `rateLimit` option, 10 / min     | user id | state growth                                                                                                                                                                                                             |
| `createPasskeyHandlers` `registrationOptions`                                                                                                                                                                             | none                                     | —       | overwrites the caller's own challenge entry (keyed by user id)                                                                                                                                                           |
| `createPushSubscriptionHandler` `DELETE`                                                                                                                                                                                  | none                                     | —       | removes a row the caller owns                                                                                                                                                                                            |
| `createNotificationsHandlers` `read` / `readAll` / `item.DELETE`                                                                                                                                                          | none                                     | —       | changes or removes rows the caller owns                                                                                                                                                                                  |
| `createPreferencesHandler` `GET`, `createNotificationsHandlers` `list`, `createSessionsHandlers` `list`, `createPasskeyHandlers` `list`, `createMeHandler`, `createInvitationHandlers` `GET`, `createStreamHandler` `GET` | none                                     | —       | reads                                                                                                                                                                                                                    |
| `createUpdateProfileHandler` `POST`                                                                                                                                                                                       | none                                     | —       | changes the caller's own row                                                                                                                                                                                             |
| `createSessionsHandlers` `revoke` / `revokeOthers`                                                                                                                                                                        | none                                     | —       | revokes the caller's own sessions                                                                                                                                                                                        |
| `createLogoutHandler` `POST`                                                                                                                                                                                              | none                                     | —       | ends the caller's own session                                                                                                                                                                                            |
| `createPasskeyHandlers` `item.PATCH` / `item.DELETE`                                                                                                                                                                      | none                                     | —       | changes or removes a row the caller owns                                                                                                                                                                                 |
| `createTwoFactorHandlers` `setup`                                                                                                                                                                                         | none                                     | —       | changes the secret on the caller's own row                                                                                                                                                                               |
| `createTwoFactorHandlers` `enable`                                                                                                                                                                                        | none                                     | —       | checks a code against a secret the same session was just handed by `setup` — nothing to guess (what its absent limiter costs an operator is in the [key-rotation runbook](#key-rotation-runbook-twofactorencryptionkey)) |
| `createInvitationHandlers` `POST`                                                                                                                                                                                         | none                                     | —       | behind the consumer's `authorize` gate (who may mint, and how many, is decided where the gate is)                                                                                                                        |
| `createInvitationHandlers` `DELETE`                                                                                                                                                                                       | none                                     | —       | behind the consumer's `authorize` gate (who may revoke is decided where the gate is)                                                                                                                                     |

What `rateLimit.passkeyRegister` bounds, and why no per-user credential cap stands beside it, is stated once beside the number itself: `RATE_LIMIT_DEFAULTS.passkeyRegister` in `server/security-defaults.ts`.
