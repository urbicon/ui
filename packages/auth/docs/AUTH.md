...
| `passkeyRegister` | **1 min** | 10 | the authenticated half of the same feature, keyed by **user id**: enrolling a passkey grows the credential table, and `registrationOptions` is the single door to that growth — `verifyRegistration` consumes the challenge its options call stored, so credential rows can never outrun this budget. Enrolling a device is a rare action, so this is a fleet's worth per minute and a script's wall |
...
- **What an authenticated write is limited for, and what it is deliberately not limited for** — the package's limiters guard three things: **state growth** (a write that can create or reassign rows), **secret verification** (a write that checks a secret the session does not itself hold — the current password on a hijacked session, a refresh token, a pending-2FA code — and is therefore a guessing target) and **third-party cost** (a write that sends a mail or a push). Three they deliberately leave alone: a write that only flips or removes rows the caller already owns, which is bounded by what the caller owns; a read; and anything behind the consumer's **`authorize` gate**, where who may do it and how often is the consumer's policy — the package does not second-guess a gate it does not define. The request rate of an authenticated caller is not this package's brake — that is the edge's, the same line the login limiter already draws above ("brakes failures, not cost"). A list of the limits that exist is therefore not a list of what is covered, so here is both halves at once (the unauthenticated per-IP flows — login, register, the token flows, `/2fa/verify`, passkey authentication — are the bullets above):
  ...
  | `createPasskeyHandlers` `registrationOptions`                                                                                                                                                | `rateLimit.passkeyRegister`, 10 / min    | user id | state growth (the ceremony's entry; each verify consumes one challenge)                                                                                                                                                  |
  ...
  | `createPasskeyHandlers` `registrationVerify`                                                                                                                                                 | none                                     | —       | consumes a challenge the limited entry issued                                                                                                                                                                            |
  | `createPushSubscriptionHandler` `DELETE`                                                                                                                                                     | none                                     | —       | removes a row the caller owns                                                                                                                                                                                            |
  | `createNotificationsHandlers` `read` / `readAll` / `item.DELETE`                                                                                                                             | none                                     | —       | flips or removes rows the caller owns                                                                                                                                                                                    |
  | `createPreferencesHandler` `GET`, `createNotificationsHandlers` `list`, `createSessionsHandlers` `list`, `createPasskeyHandlers` `list`, `createMeHandler`, `createInvitationHandlers` `GET` | none                                     | —       | reads                                                                                                                                                                                                                    |
  | `createUpdateProfileHandler` `POST`                                                                                                                                                          | none                                     | —       | updates the caller's own row                                                                                                                                                                                             |
  ...
  | `createPasskeyHandlers` `item.PATCH` / `item.DELETE`                                                                                                                                         | none                                     | —       | renames or removes a row the caller owns                                                                                                                                                                                 |
  | `createTwoFactorHandlers` `setup`                                                                                                                                                            | none                                     | —       | overwrites the secret on the caller's own row                                                                                                                                                                            |
